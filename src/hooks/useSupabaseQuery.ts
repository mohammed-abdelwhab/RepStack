...
// Seed basic workouts for a new user
export async function seedNewUserWorkouts(userId: string): Promise<void> {
  // Push
  const pushDay = await createWorkoutDay(userId, "Push", 0);
  await addExercise(
    pushDay.id,
    "Incline Chest Press",
    "Focus on upper chest chest stretch",
    null,
    0,
    3,
  );
  ...
  // Pull
  const pullDay = await createWorkoutDay(userId, "Pull", 1);
  await addExercise(
    pullDay.id,
    "Weighted Pull-Up",
  ...
  await addExercise(pullDay.id, "Barbell Row", "Keep back flat", null, 1, 3);

  // Legs
  const legsDay = await createWorkoutDay(userId, "Leg", 2);
  await addExercise(
    legsDay.id,
    "Barbell Squat",
  ...
  await addExercise(
    legsDay.id,
    "Romanian Deadlift",
    "Hinge at the hips, stretch hamstrings",
    null,
    1,
    3,
  );
